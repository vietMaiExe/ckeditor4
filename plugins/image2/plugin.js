 ...
 * For licensing, see LICENSE.md or https://ckeditor.com/legal/ckeditor-oss-license123
 */
...
(function () {
	...
	CKEDITOR.plugins.add("image2", {
		...
		init: function (editor) {
			...
			// got to be refined.
			config.filebrowserImage2BrowseUrl =
				config.filebrowserImageBrowseUrl;
			config.filebrowserImage2UploadUrl =
			...
			init: function () {
				var helpers = CKEDITOR.plugins.image2,
					...
					data = {
						...
						lock:
							editor.config.image2_defaultLockRatio !== undefined
								? editor.config.image2_defaultLockRatio
								: legacyLockBehavior,
					};
	...
	function linkCommandIntegrator(editor) {
		...
		var listener = CKEDITOR.on("dialogDefinition", function (evt) {
			...
			if (dialog.name == "link") {
				...
				def.onShow = function () {
					var widget = getFocusedWidget(editor),
						displayTextField = this.getContentElement(
							"info",
							"linkDisplayText"
						)
							.getElement()
